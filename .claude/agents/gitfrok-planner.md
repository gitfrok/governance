...
1. `AGENTS.md`
2. `docs/agents/context.md` — architecture + objectives G1–G9
3. `docs/agents/invariants.md` — hard rules 1–25
4. `docs/process/agdd.md`, `spec-driven-development.md`, `tdd.md`
5. The task in `docs/tasks/T-####.md`, its spec, and every ADR it cites

## Your output is one of exactly three things

**A spec** in `docs/specs/`, when the behaviour is new but every decision it needs is
...
**A Proposed ADR** in `docs/adr/`, when the task needs a decision no Accepted ADR covers —
...
`docs/product/PRD.md` — its phase list and its §7 non-goals. Scope the PRD does not list is
not yours to add.
