...
- **Status:** Approved (2026-08-19) — ADR-0076 Accepted with this increment; RED may begin
- **Owner:** platform
...
- [ ] **AC1** `GetSettings` returns one repository's name, description, archived state and the
...
- [ ] **AC2** `UpdateSettings` changes name and description and nothing else. The name is required
...
- [ ] **AC3** `SetArchived` sets or clears the archived instant. Archiving an archived repository is
...
- [ ] **AC4** **Every accepted change appends exactly one audit record**, naming the actor, the
...
- [ ] **AC5** **A settings change is refused unless the PDP allows it**, and the refusal is a coarse
...
- [ ] **AC6** **The audit record is written through a port this context declares**, not by importing
...
- [ ] **AC7** **Archival changes no authorization or read outcome.** A test archives a repository and
      ...
      executable form.
- [ ] **AC8** The new columns are additive over the existing registry; the migration passes T-0004's
...
- [ ] **AC9** **The isolation proofs ran.** Zero skips for the tenancy cases; the exit record states
      the observed skip count (carried limit 5).

### The wire and the BFF (T-0069)

- [ ] **AC10** Additive: `buf breaking` passes; `RepositorySettings` is a new service and no existing
      message changes shape.
- [ ] **AC11** **No message in `repository/v1` carries a visibility, membership or policy field.** A
...
- [ ] **AC12** **No delete verb reaches the repository surface.** The same check asserts no RPC named
...
- [ ] **AC13** The BFF shapes and forwards under the session: a read, a settings write, and an
...
- [ ] **AC14** The BFF response body carries no visibility, member, role or protection vocabulary. A
      test asserts it, so the accepted increment holds at the layer a browser actually reads rather
      than only in the contract.

### The view (T-0070)

- [ ] **AC15** The settings page shows name, description and archived state, each editable by a plain
      form that works with no client script, and states who last changed the settings and when.
- [ ] **AC16** **The page states what it does not do, and the copy enumeration forbids the softer
...
- [ ] **AC17** **An archived repository says what archival does and does not do**: the label is
...
- [ ] **AC18** Description renders as text, not as HTML. A description containing markup is
      displayed, not executed — the same rule SPEC-0056 AC14 set for release notes, and for the same
      reason.
- [ ] **AC19** No hex literal; every status word in `src/lib/status.ts` with a glyph and a word;
      archived and active are separable in grayscale and under deuteranopia; the two regression pins
      unmodified.
- [ ] **AC20** The stub serves a plain repository and an archived one; captures regenerated per
