...
- **ADRs:** 0062 (decides durability), 0003 (RLS tenant scoping), 0023 (PostgreSQL 18), 0025 (module-owned migrations), 0055 (growth metering stays a follow-up)
- **Task(s):** — (Phase 3.1, epic EP-19; task to be filed)
...
- [ ] AC1: Enrolment-token spend and revocation state survives a control-plane kill-and-restart. A
  ...
  connection. Single-use is a property of the platform, not of a process.
...
- [ ] AC5: Migrations are additive and rollback-tested (up/down verified); every new table carries
  RLS tenant scoping with no un-tenant-scoped query path (ADR-0003); and enrolment tokens persist as
  one-way hashes only — never persisted, logged or recoverable in raw form (SPEC-0038 AC2, now
  applied to the store as well as the logs).
...
- Migration up/down tests per module (AC5), plus an RLS assertion query per new table.

## Governance mapping (G1–G9)

| Objective | How this spec satisfies it |
|---|---|
| G1 Tenant isolation | RLS on every new table; no un-tenant-scoped query path exists (AC5) |
