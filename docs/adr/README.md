...
| [ADR-0081](0081-ci-job-logs-deferral.md) | CI job logs remain deferred — the reopen trigger is a user asking, an observed bypass, or an owned redaction capability, not the PRD row; capture and residency are fixed now and PR-26's scoping clause becomes an access floor, so the reopen starts two questions smaller; check 13 keeps holding the absence at the wire | Proposed |
| [ADR-0082](0082-tenant-policy-authoring-deferral.md) | Tenant policy authoring remains deferred — reading A stays the only authoring path; the reopen trigger is a named tenant need, not the PRD row; check 14 keeps holding the absence at the wire | Proposed |
...
| [ADR-0084](0084-version-guard-and-the-projection-write.md) | Save has two shapes and the version guard can only guard one — the event path's version-preserving projection write conflicts with AC9's guard on every push; proposes splitting the write, restoring the conflict's error, ordering Merge's save before its ref move, and letting the idempotency insert converge; T-0078's RED stops until this is decided | Proposed |
