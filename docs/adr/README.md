...
| [ADR-0094](0094-cross-plane-read-path.md) | The repository surface is served data-plane-side, behind a door the customer exposes — the gap underneath ADR-0093's: the BYO chart renders no Service and `git.gitsaas.test` exists only in `deploy/dev`, so `git push` was as unreachable as a file view and the door itself had never been decided. The chart still opens nothing; the customer exposes one door serving Git and the repository surface alike, a `bff`+`webfrontend` pair moves data-plane-side by route partition, and cross-plane navigation is a URL rather than a proxy — so source never enters a vendor process and ADR-0011 stands unamended. Amends ADR-0093's partition | Proposed |
...
| **The cross-plane repository read path.** The BFF is control-plane-side and requires the RepositoryReader that `git-storaged` serves data-plane-side, exiting without it; ADR-0011 permits no inbound path to a data plane and no relay decision exists. `deploy/dev` hides it by collapsing both planes into one namespace. **Blocks the browser surface in any real two-cluster deployment** | 0093, 0011 |
