...
| **Amend SPEC-0021 and every repository-surface spec for the data-plane premise**, and file the implementing task. Request and response shapes are unchanged; the deployment premise is not. **Nothing moves in `bff/` or `webfrontend/` until this lands** | 0094, 0021 |
...
| **Production install of the third-party stateful set** ADR-0093 decision 2 declares as chart inputs — Postgres, Valkey, Redpanda, OpenBao, Zitadel — by upstream charts, operators or promoted manifests. `deploy/dev` is Minikube's and is not it. **Blocks a first deployment** | 0093, 0092 |
