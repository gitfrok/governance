...
| T-0091 | A gate that refuses an inherited cluster location | ADR-0106 carry | **Todo** — SPEC-0072 AC1-AC7. ADR-0106 conceded "no gate enforces it" as its own weakest point: `var.location` defaults to the region, so a `gke` unit with the line DELETED is valid HCL that plans clean and applies into a regional cluster creating `min_nodes` nodes PER ZONE. The failure is an omission, not an error, and the bill arrives a month later. Asserts EXPLICITNESS and never a value — AC2 (a region is equally accepted) is what forbids the cheap grep-for-the-zone implementation and keeps ADR-0106 decision 2's reversal path a one-line edit rather than a gate fight |
