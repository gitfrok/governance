...
| T-0092 | The debt the first real deployment took on | ADR-0106 / ADR-0107 carry | **Todo** — a ledger, not a plan: twelve items taken knowingly during the bring-up that first made `git clone` work on `git-gitfrok.7.solutions`. Three make the product wrong rather than incomplete: a PAT dies on every data-plane restart (`identity.NewPostgres` exists and nothing calls it), every OpenBao restart re-seals, and the git volume is `standard-rwo` against ADR-0106 decision 4. Closed by being emptied into real tasks |
