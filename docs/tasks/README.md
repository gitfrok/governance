...
| T-0087 | The BFF's plane partition | ADR-0094 carry | **Done (2026-09-22)** — bff@ec3d6c9 + 3c02149; **9/10**. Both refusals, 11 control + 26 data-plane registrations, manifests gated, and AC8's no-dial rule in bff's own arch package (invariant 22 forbids backend walking it). **AC7 blocked** — no enrolment-record field |
| T-0088 | Register the control plane's four service doors | ADR-0094/0100 carry | **Done (2026-09-22)** — backend@92c8acf; three of four on a new BFF door, door set asserted as a set (5→8). OIDCLogin absent by decision, its absence tested. Also fixed: this plane was using the MEMORY decision store with a pool in hand. The AC7 assertion caught ADR-0100's table claiming 14 data-plane services where there are 13 |
| T-0089 | The custody client can be given a CA, and refuses to guess | ADR-0104 carry | **Done (2026-09-22)** — backend@7a8dccd; SPEC-0071 AC1–AC7, `-race` green with 0 skips. Both critical assertions mutation-proven. AC2's first test was unsound — `SystemCertPool().Subjects()` is empty on darwin, so a substituting mutant passed a subject count; now `CertPool.Equal`. Also found what the ACs missed: `KubernetesAuth` dials separately and needed the CA too |
| T-0090 | The installer mounts the custody CA, and a gate keeps the pair together | ADR-0104 carry | **Done (2026-09-22)** — super-repo@a3d59c3; SPEC-0071 AC8–AC12; cp-kustomize-test 10→14 assertions plus the harness's first positive fixture. The three new fixtures were **decorative on first writing** — refused by AC8 on their own directory name before the rule existed — and are now nested so each fails for exactly its own reason. Same weakness noted, unfixed, in T-0084's nine |
| T-0091 | A gate that refuses an inherited cluster location | ADR-0106 carry | **Done (2026-09-22)** — super-repo@02dc44d; SPEC-0072 AC1-AC7, `make verify` 0. **The stub is the finding**: the cheapest implementation satisfying AC1 (grep for the shipped zone) passed `missing-location` AND both live units, and was caught only by the `regional` fixture — a suite written from refusals alone ships it, and it fails the day someone restores regional under an availability requirement. Three mutations each caught by exactly their own fixture; also proven against the real units with one line deleted. Harness reads violation TEXT, per T-0090. Noted out of scope: `make lint-shell` was already red (SC2259 in two kustomize gates) |
| T-0092 | The debt the first real deployment took on | ADR-0106 / ADR-0107 carry | **Todo** — a ledger, not a plan: twelve items taken knowingly during the bring-up that first made `git clone` work on `git-gitfrok.7.solutions`. Two make the product wrong rather than incomplete: a PAT dies on every data-plane restart (`identity.NewPostgres` exists and nothing calls it) and every OpenBao restart re-seals. A third — the git volume on `standard-rwo` against ADR-0106 decision 4 — was migrated to `premium-rwo` the same day Closed by being emptied into real tasks |
| T-0093 | A data-plane BFF logs people in | ADR-0102 / EP-32 carry | **Done (2026-09-23)** — bff@14adef6; SPEC-0073 AC1–AC6, AC6 on minikube: login roundtrip, session with roles [owner], repository list 200 with the session and 404 without |
| T-0094 | Reading notifications is denied for every principal | EP-31 carry | **Todo** — the policy grants `notification.read` by role and the notifications contract carries no roles, so the PDP always denies; found while proving T-0093 |
