...
| T-0090 | The installer mounts the custody CA, and a gate keeps the pair together | ADR-0104 carry | Todo — SPEC-0071 AC8–AC12; super-repo; **blocked-by T-0089**. Mounts the operator-created `openbao-ca` (never `openbao-tls`, which holds the server key) and gates that an `https` custody address and a mounted CA always travel together |
