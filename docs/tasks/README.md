...
| T-0092 | The debt the first real deployment took on | ADR-0106 / ADR-0107 carry | **Todo** — a ledger, not a plan: ten items taken knowingly during the bring-up that first made `git clone` work on `git-gitfrok.7.solutions`. Two make the product wrong rather than incomplete: a PAT dies on every data-plane restart (`identity.NewPostgres` exists and nothing calls it) and every OpenBao restart re-seals. Closed by being emptied into real tasks |
