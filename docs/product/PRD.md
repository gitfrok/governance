...
| PR-26 | A developer can see pipeline runs and job logs for a repository, scoped by the same permissions as the repository read | G1 | 0070, 0005 | **Delivered in part (2026-08-19)** — SPEC-0054, T-0059…T-0061. The runs half ships; **job logs are NOT delivered** — job output does not exist (ADR-0072), and ADR-0081 keeps them deferred with named reopen triggers, held by check 13 rather than by intention (§12.1) |
| PR-27 | A policy owner can author, version, dry-run and enforce a policy from the web UI, with the deciding version recorded | G4, G6 | 0070, 0006 | **Delivered in part (2026-08-19)** — SPEC-0055, T-0062/T-0063. The visibility half ships; **authoring is NOT delivered** — reading A (governance-PR authoring) is the approved reading per SPEC-0029, ADR-0073 fixed the narrow-only boundary, and ADR-0082 keeps in-product authoring deferred with its reopen trigger, held by check 14 rather than by intention (§12.1) |
...
| PR-29 | A team can cut and publish a release from a tag, with its artifacts and notes | — | 0070, 0075 | **Delivered in part (2026-08-19)** — SPEC-0056, T-0064…T-0066. Tags and notes ship; **artifacts are NOT delivered**, held by check 15; ADR-0083 names the reopen triggers and fixes the storage shape for that day (§12.1) |
...
| PR-29 artifacts | releases | **Closed as deferred 2026-08-19**: ADR-0075 Accepted the tags-and-notes increment and check 15 holds the absence at the wire. Artifacts re-open signing, custody, retention and metering at once. **Dispositioned 2026-08-21 by ADR-0083 (Accepted)**: reopen triggers named (a user asks, tenant custody gains a signing consumer, or import parity names release assets) and the shape fixed — tenant-scoped SeaweedFS blob storage with its own retention class |
| PR-26 job logs | pipeline runs | **Closed as deferred**: ADR-0072 kept output out of the runs surface (the sandbox is destroyed at job end; nothing was ever asked to keep it). **Dispositioned 2026-08-21 by ADR-0081 (Accepted)**: what reopens the logs is a user asking, an observed bypass, or redaction becoming an owned capability — never the PRD row's silence; capture, redaction, retention and size, access and residency are the questions a reopen must answer, and check 13 holds the absence until then |
| PR-27 authoring | policy | **Closed as deferred**: SPEC-0029 settled reading A (governance-PR authoring) at approval; ADR-0073 found authoring absent structurally and fixed one boundary — a tenant policy may only narrow, never widen. **Dispositioned 2026-08-21 by ADR-0082 (Accepted)**: reading B stays deferred, the reopen trigger is a named tenant need rather than the PRD row, and check 14 holds the absence at the wire |
| Notifications | — | **New work implied 2026-08-21** by ADR-0086 (Accepted): the product emits events but nothing tells an actor something happened. First channel in-app (list + unread count + mark-read); email/webhooks are named follow-ups. SPEC-0063 Approved; T-0080 filed under EP-31 |
| Four-eyes floor · draft MRs · merge strategies | PR-9's review loop | **New work implied 2026-08-21** by ADR-0085/0087/0088 (all Accepted): every merge needs two non-author approvals platform-wide; merge requests can be drafts with one door out; merges land as merge_commit/squash/rebase per repository with trunk-based landing as a mode. SPEC-0062/0064 Implemented (T-0079/T-0081); SPEC-0065 Approved (T-0082) |
