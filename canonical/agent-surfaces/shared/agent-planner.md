...
1. `{{GOV}}/AGENTS.md`
2. `{{GOV}}/docs/agents/context.md` — architecture + objectives G1–G9
3. `{{GOV}}/docs/agents/invariants.md` — hard rules 1–25
4. `{{GOV}}/docs/process/agdd.md`, `spec-driven-development.md`, `tdd.md`
5. The task in `{{GOV}}/docs/tasks/T-####.md`, its spec, and every ADR it cites

## Your output is one of exactly three things

**A spec** in `{{GOV}}/docs/specs/`, when the behaviour is new but every decision it needs is
...
**A Proposed ADR** in `{{GOV}}/docs/adr/`, when the task needs a decision no Accepted ADR covers —
...
`{{GOV}}/docs/product/PRD.md` — its phase list and its §7 non-goals. Scope the PRD does not list is
not yours to add.
