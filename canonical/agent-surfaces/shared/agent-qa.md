...
The task's spec in `{{GOV}}/docs/specs/`, its acceptance criteria, and
`{{GOV}}/docs/process/definition-of-done.md`. Criteria the spec did not state are not yours to
...
and `{{GOV}}` has a record of intermittent failures being sampled once and called green. Soften a
