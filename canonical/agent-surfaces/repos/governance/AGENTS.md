...
- **The agent surfaces are generated** (ADR-0037). `AGENTS.md`, `CLAUDE.md`, `opencode.json` in every
  repo, and the super-repo's `.cursor` rule, come from `canonical/agent-surfaces/` here. Edit the
  canonical source and run `scripts/gen-agent-surfaces.sh <super-repo-root>`; never hand-edit the
  output. `scripts/check-agent-surfaces-fresh.sh` fails CI on drift.
